...
## 2名基準への統一（2026-08-15）
- たつの・西播磨沿岸市場は旅館・リゾート比率が高く、1名利用を販売しない施設が多い。
- そのため、カレンダー・履歴グラフ・需要シンボル・需要急騰履歴を **2名1室基準** に統一。
- 1名/2名切替タブは廃止。
- Daily Updateも2名検索のみ実行し、APIリクエスト数と実行時間を削減。
- 需要シンボルは2名データの「販売中施設数 OR 平均価格」の絶対値で判定。
